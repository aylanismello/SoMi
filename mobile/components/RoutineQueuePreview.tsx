...
export default function RoutineQueuePreview(): React.JSX.Element {
  ...
  const {
    ...
    isEditMode?: boolean
    onQueueUpdate?: ((queue: any) => void) | null
    displayMinutes?: number | null
    reasoning?: string | null
  }

  const [enrichedQueue, setEnrichedQueue] = useState<(SomiBlockSegment & { _segmentIndex: number })[]>([])
  const [libraryBlocks, setLibraryBlocks] = useState<SomiBlock[]>([])
  ...
  useEffect(() => {
    const loadLibrary = async () => {
      ...
      setLibraryBlocks((library || []) as SomiBlock[])
    }
    loadLibrary()
  }, [])
  ...
  const handleBlockSelect = (libraryBlock: SomiBlock) => {
    ...
    const swapData = {
      ...
      energy_delta:   libraryBlock.energy_delta ?? 0,
      safety_delta:   libraryBlock.safety_delta ?? 0,
    }
    ...
    updateSegment(newQueue[swapIndex]._segmentIndex, swapData as Partial<SomiBlockSegment>)
  ...
  // ── Block renderer ──────────────────────────────────────────────────────────
  const renderBlock = (block: SomiBlockSegment & { _segmentIndex: number }, index: number) => {
  ...
  const renderBlockList = () => {
    ...
    if (hasSections) {
      const groups: { name: string; items: { block: SomiBlockSegment & { _segmentIndex: number }; globalIndex: number }[] }[] = []
