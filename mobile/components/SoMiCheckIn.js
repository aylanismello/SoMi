...
export default function SoMiCheckIn({ navigation, route }) {
  ...
  // Store in state to persist through navigation
  // Check route params first, then fall back to routine store (!isQuickRoutine means it's daily flow)
  const [isDailyFlow] = useState(route?.params?.isDailyFlow === true || !routineStore.isQuickRoutine)
  ...
  const handleContinueToExercise = () => {
    ...
    // Transition back to Step 2 for another round
  ...
  const handleFinish = async () => {
    ...
    console.log('🔍 fromPlayer value:', fromPlayer)
    console.log('🔍 fromBodyScan value:', fromBodyScan)
    ...
    console.log('🔄 handleFinish called - starting...')

    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium)

    // Save loop check to database
    console.log('💾 Saving embodiment check...')
    await saveEmbodimentCheck(loopSliderValue, loopPolyvagalState, loopJournalEntry || null)
    console.log('✅ Embodiment check saved')

    // End the active chain when done
    console.log('🏁 Ending active chain...')
    ...
    const isFinalCheckIn = currentStep === 4 || fromPlayer // Step 4 is the final check-in after routine
    console.log(`📊 Flow type check: isDaily=${isDaily}, isFinalCheckIn=${isFinalCheckIn}, currentStep=${currentStep}, fromPlayer=${fromPlayer}, routineStore.isQuickRoutine=${routineStore.isQuickRoutine}`)

    // Reset routine store for next session
    console.log('🔄 Resetting routine store...')
    routineStore.resetRoutine()
    console.log('✅ Routine store reset')

    // Navigate based on flow type
    if (isDaily && isFinalCheckIn) {
      // Daily flow completion - show big celebration screen
      console.log('✅✅✅ Navigating to CompletionScreen (Daily Flow)')
      navigation.navigate('CompletionScreen')
    } else {
      // Quick routine or other flows - go home
      console.log('❌ Navigating to Home (Quick Routine)')
      console.log(`Reason: isDaily=${isDaily}, isFinalCheckIn=${isFinalCheckIn}, currentStep=${currentStep}`)
