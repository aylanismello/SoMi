...
export default function CompletionScreen(): React.JSX.Element {
  ...
  const totalPlaySeconds = (latestChain?.duration_seconds ?? 0) > 0
    ? (latestChain?.duration_seconds ?? 0)
    : (latestChain?.somi_chain_entries || []).reduce((sum: number, e) => sum + (e.seconds_elapsed || 0), 0)
