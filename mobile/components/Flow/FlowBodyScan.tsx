...
export default function BodyScanCountdown(): React.JSX.Element {
  ...
  const {
    ...
    savedInitialState?: any
    skipToRoutine?: boolean
    fromCheckIn?: boolean
    finalOrderIndex?: number
    duration_seconds?: number
  }

  const durationSeconds = duration_seconds ?? 60
  ...
  const pausedCountdownRef = useRef<number>(durationSeconds)
  ...
  const handleComplete = async () => {
    ...
    if (skipToRoutine) {
      ...
      ;(navigation as any).replace('FlowOutro', { fromBodyScan: true })
    } else {
  ...
  const handlePauseResume = () => {
    if (!isPaused) {
      ...
      const elapsed = durationSeconds - resumeCountdown
      const remaining = resumeCountdown * 1000
      progressAnim.setValue(elapsed / durationSeconds)
      Animated.timing(progressAnim, {
        toValue: 1,
        duration: remaining,
        useNativeDriver: false,
