...
import { StyleSheet, View, Text, Dimensions, PanResponder, LayoutChangeEvent } from 'react-native'
...
interface PolyvagalStatePickerProps {
  energyLevel: number
  safetyLevel: number
...
export default function PolyvagalStatePicker({
  ...
  onDragEnd,
  hideReadout,
}: PolyvagalStatePickerProps): React.JSX.Element {
  ...
  const onEnergyRef = useRef<typeof onEnergyChange>(onEnergyChange)
  const onSafetyRef = useRef<typeof onSafetyChange>(onSafetyChange)
  const onDragStartRef = useRef<typeof onDragStart>(onDragStart)
  ...
  const panResponder = useMemo(() => {
    function apply(evt: { nativeEvent: { locationX: number; locationY: number } }) {
      ...
      >
        ...
        {[
          ...
          <View
            key={i}
            pointerEvents="none"
            style={[styles.axisCorner, corner.pos, { alignItems: corner.align as 'flex-start' | 'flex-end' }]}
          >
