...
import type { SomiBlock, SomiBlockSegment } from '../../types'
...
export default function EditFlowScreen(): React.JSX.Element {
  ...
  const [allBlocks, setAllBlocks] = useState<SomiBlock[]>([])
  ...
  const queue = segments.filter((s): s is SomiBlockSegment => s.type === 'somi_block')
  ...
  const handleSwap = (newBlock: Partial<SomiBlockSegment>) => {
  ...
  const groupedBlocks = (() => {
    ...
    if (!hasSections) return [{ name: null as string | null, items: filteredBlocks }]
    const groups: Record<string, SomiBlock[]> = {}
  ...
  const renderBodyScanToggle = (which: 'start' | 'end') => {
  ...
  const renderBlockRow = (block: SomiBlockSegment, globalIndex: number) => {
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        onPress={() => handleSwap({
                          ...
                          energy_delta: block.energy_delta ?? undefined,
                          safety_delta: block.safety_delta ?? undefined,
                          url: block.media_url ?? undefined,
                          section: (block.section as SomiBlockSegment['section']) ?? undefined,
