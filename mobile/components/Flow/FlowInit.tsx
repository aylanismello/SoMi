...
function DurationPickerModal({ visible, minutes, onClose, onSave }: DurationPickerModalProps): React.JSX.Element {
...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function DailyFlowSetup(): React.JSX.Element {
  ...
  const energyRef  = useRef<number>(50)
  const safetyRef  = useRef<number>(50)
  const hasInitializedRef      = useRef<boolean>(false)
  const isReadyForInputRef     = useRef<boolean>(false)
  const pendingRegenRef        = useRef<boolean>(false)

  const glowAnim  = useRef(new Animated.Value(0)).current
  const toastAnim = useRef(new Animated.Value(-120)).current
  const [isToastVisible, setIsToastVisible] = useState(false)
  const isToastShownRef      = useRef<boolean>(false)
  ...
  const doGeneratePreview = useCallback(async (
    durationMinutes: number, energy: number, safety: number, isInitial: boolean,
    scanStartOverride?: boolean, scanEndOverride?: boolean
  ) => {
    if (isInitial) {
  ...
  const handleEditRoutine = () => {
    ...
    const now = new Date()
    useEditFlowStore.getState().setGenerationParams({
      polyvagal_state: deriveState(energyLevel, safetyLevel).name,
  ...
  const handleStartFlow = async () => {
    ...
    useRoutineStore.getState().initializeRoutine({
      ...
      savedInitialValue:  0,
      savedInitialState:  derivedState,
      segments:           segments,
    ...
    if (hasBodyScanStart) {
      ...
      ;(navigation as any).replace('FlowBodyScan', { isInitial: true, skipToRoutine: true })
    } else {
      ;(navigation as any).replace('SoMiRoutine')
    }
  }
  ...
  const handleDurationSave = useCallback((min: number) => {
