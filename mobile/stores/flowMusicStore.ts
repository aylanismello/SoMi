...
import type { FlowMusicStore, Track, TrackId, AudioPlayer } from '../types'
...
function volumeAnimFor(trackId: TrackId): Animated.Value | null {
  if (trackId === 'fluids') return fluidsVolumeAnim
  if (trackId === 'together') return togetherVolumeAnim
  return null
}

function playerFor(state: FlowMusicStore, trackId: TrackId): AudioPlayer | null {
...
function startFade(volumeAnim: Animated.Value, player: AudioPlayer, toValue: number, duration: number, onComplete?: () => void) {
...
export const useFlowMusicStore = create<FlowMusicStore>((set, get) => ({
