import type { PolyvagalStatesMap, PolyvagalState, PolyvagalExplanation, PolyvagalZone } from '../types'

export const POLYVAGAL_STATES: PolyvagalStatesMap = {
...
export function deriveState(energy: number | null, safety: number | null): PolyvagalState {
  const e = energy ?? 50
  const s = safety ?? 50
...
export function deriveIntensity(energy: number | null, safety: number | null): number {
  const e = energy ?? 50
  const s = safety ?? 50
...
export function deriveStateFromDeltas(energy_delta: number | null, safety_delta: number | null): PolyvagalState {
...
const POLYVAGAL_EXPLANATIONS: PolyvagalZone[] = [
  {
    id: 'deep_rest',
    test: (e, s) => s > 50 && e < 25,
    title: 'Deep Rest',
...
export function getPolyvagalExplanation(energy: number | null, safety: number | null): PolyvagalExplanation {
  const e = energy ?? 50
  const s = safety ?? 50
