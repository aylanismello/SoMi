...
import type { SomiBlock, SomiChain, FlowType } from '../types'

export const QUERY_KEYS = {
  blocks: (canonicalNames?: string[]) => ['blocks', ...(canonicalNames || [])],
  chains: ['chains'],
 ...
 */
export function useBlocks(canonicalNames?: string[]) {
  return useQuery({
...
export function useSaveEmbodimentCheck() {
  ...
  return useMutation({
    mutationFn: async ({ energyLevel, safetyLevel, journalEntry = null, flowType = null, tags = null }: { energyLevel: number; safetyLevel: number; journalEntry?: string | null; flowType?: FlowType | null; tags?: string[] | null }) => {
...
export function useSaveChainEntry() {
  ...
  return useMutation({
    mutationFn: async ({ somiBlockId, secondsElapsed, orderIndex = 0, chainId = null, flowType = null, section = null }: { somiBlockId: number; secondsElapsed: number; orderIndex?: number; chainId?: number | null; flowType?: FlowType | null; section?: string | null }) => {
      // Get flowType from routine store if not provided
      const finalFlowType = flowType || useRoutineStore.getState().flowType
