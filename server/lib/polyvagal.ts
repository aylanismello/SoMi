...
import type { SomiBlock, PolyvagalStateName, SectionName, Segment } from '../types'

export function filterBlocksByState(blocks: SomiBlock[], state: PolyvagalStateName): SomiBlock[] {
  const filters: Record<PolyvagalStateName, (b: SomiBlock) => boolean> = {
...
export function selectBlocks(pool: SomiBlock[], count: number): SomiBlock[] {
  if (pool.length === 0) return []

  const selected: SomiBlock[] = []
  let shuffled: SomiBlock[] = shuffle([...pool])
  let idx = 0
...
export function assignSections(blocks: SomiBlock[]): SomiBlock[] {
...
export function generateExplanation(state: PolyvagalStateName, blocks: Segment[]): string {
  const stateIntros: Record<PolyvagalStateName, string> = {
  ...
  const somiBlocks = blocks.filter((b): b is Segment & { type: 'somi_block'; name: string } => b.type === 'somi_block')
