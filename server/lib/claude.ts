...
import { formatContextForPrompt } from './sessionContext'
import type { GenerateAIRoutineParams, AIRoutineResult } from '../types'
...
export async function generateAIRoutine({ sessionContext, availableBlocks, blockCount: explicitBlockCount, hasScanStart = false, hasScanEnd = false }: GenerateAIRoutineParams): Promise<AIRoutineResult> {
  const blockCount: number = explicitBlockCount ?? Math.round(sessionContext.duration_minutes)
  ...
  let structureRequirements: string
  if (blockCount === 1) {
...
Apply polyvagal principles for the ${sessionContext.polyvagal_state} state. Use the contextual signals to refine your choices.`
  ...
  const cleaned: string = rawText.replace(/^```(?:json)?\s*/i, '').replace(/\s*```\s*$/i, '').trim()
  const parsed: AIRoutineResult = JSON.parse(cleaned)
  ...
  const availableSet: Set<string> = new Set(availableBlocks)
  parsed.sections = parsed.sections.map((section: AIRoutineResult['sections'][number]) => ({
    ...section,
    blocks: section.blocks.filter((b: { canonical_name: string }) => availableSet.has(b.canonical_name)),
  }))
