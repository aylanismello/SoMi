...
import type { SomiBlock, Segment, SectionName, PolyvagalStateName, GenerateFlowRequestBody } from '../../../../types'
...
let _blocksCache: SomiBlock[] | null = null
let _blocksCacheAt: number = 0
...
export async function POST(request: NextRequest) {
  ...
  try {
    const body: GenerateFlowRequestBody = await request.json()
    const {
      polyvagal_state,
      duration_minutes,
    ...
    let allBlocks: SomiBlock[] | null = _blocksCache
    const now = Date.now()
    if (!allBlocks || now - _blocksCacheAt > BLOCKS_CACHE_TTL_MS) {
      ...
      _blocksCache = data as SomiBlock[]
      _blocksCacheAt = now
      allBlocks = data as SomiBlock[]
    }
...
function algorithmicPath(allBlocks: SomiBlock[], polyvagal_state: PolyvagalStateName, block_count: number, scanStart: boolean, scanEnd: boolean): { segments: Segment[]; reasoning: string } {
...
function assembleSegments(blocksWithSections: SomiBlock[], scanStart: boolean, scanEnd: boolean): Segment[] {
